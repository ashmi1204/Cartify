...
public class CartfyGUI {
    ...
    public static void createAndShowGUI() {
        ...
        if (products != null) {
            for (Product product : products) {
                productStockMap.put(product.name, product.quantity);
        ...
        checkoutButton.addActionListener(e -> {
            ...
            if (updatedProducts != null) {
                for (Product product : updatedProducts) {
                    ...
                    product.quantity = stockOverride;
                    JPanel card = createProductCard(product, cartItemsPanel, totalPrice, totalLabel, productGridPanel);
                    productGridPanel.add(card);
                }
            }
    ...
    private static JPanel createProductCard(Product product, JPanel cartItemsPanel, double[] totalPrice,
        ...
        boolean isOutOfStock = product.quantity <= 0;
        ...
        JLabel stockLabel = new JLabel(isOutOfStock ? "OUT OF STOCK" : "In Stock: " + product.quantity);
        ...
        new Thread(() -> {
            try {
                URL imageUrl = new URL(product.image);
        ...
        if (!isOutOfStock) {
            ...
            card.addMouseListener(new MouseAdapter() {
                public void mouseClicked(MouseEvent e) {
                    ...
                    product.quantity--;
                    stockLabel.setText(product.quantity <= 0 ? "OUT OF STOCK" : "In Stock: " + product.quantity);
