...
public class ProductRepo {
    ...
    public void updateStock(Long productId, int quantityChange) {
        ...
        try {
            Integer currentStock = jdbcTemplate.queryForObject(checkSql, Integer.class, productId);
            ...
            if (currentStock < quantityChange) {
                throw new IllegalStateException("Insufficient stock. Available: " + currentStock + ", Requested: " + quantityChange);
            }

            String updateSql = "UPDATE products SET product_qty = product_qty - ? WHERE product_id = ?";
            int rowsAffected = jdbcTemplate.update(updateSql, quantityChange, productId);

            if (rowsAffected == 0) {
                throw new IllegalArgumentException("Failed to update stock for product ID: " + productId);
            }
